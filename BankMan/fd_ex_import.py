# -*- coding: utf-8 -*-
# File: BankMan\fd_ex_import.py

"""fd_ex_import.py

Export and import the ``fd_master`` table (Fixed Deposit master records).

Export format
-------------
Each line is a Python tuple literal (produced by ``repr(tuple(...))``).
The first element is the row type ``'FD'``::

    ('FD', bank_key, ac_number, fd_number, principal_amount,
            interest_rate, open_dt, maturity_dt, is_active)
...
linked account portably across databases.  ``fd_number`` (UNIQUE on
``fd_master``) is used as the portable FD identifier for transaction rows.

Import behaviour
----------------
* Lines starting with ``#`` and blank lines are silently ignored.
* **FD rows (``'FD'``)**: the linked account is resolved via
  ``(bank_key, ac_number)``.  A row is skipped when an FD with the same
  ``fd_number`` already exists (the unique constraint ``UNIQUE(fd_number)``).
* DB and file errors are reported via the GUI and logged; processing
  continues for the remaining lines.

Output / input file
-------------------
``C:\\Data\\Personal\\Finance_and_Investment\\finprog\\FinanceManager\\data\\all_fd_master.txt``
...
_EXPORT_FILE = os.path.join(_DATA_DIR, "all_fd_master.txt")
_BACKUP_NAME = "all_fd_master.backup.txt"
...
def _prepare_export_file() -> str:
    ...
    Returns the absolute path to ``all_fd_master.txt``.
    """
...
def export_fd(
    ...
    """Export the ``fd_master`` table to ``all_fd_master.txt``.
    ...
    try:
        with get_db_connection(BANK_DB_PATH) as conn, open(
            ...
            cursor.execute("""
                SELECT
                    ...
                    fd.is_active
                FROM fd_master fd
                JOIN accounts a ON fd.account_id = a.ac_id
                JOIN banks    b ON a.b_id = b.b_id
                ORDER BY fd.fd_master_id
            """)
            rows = cursor.fetchall()

            if not rows:
                show_colorful_info(
                    parent, "No Data", "No FD master records found to export."
                )
                return
        ...
        logger.exception("export_fd: DB error: %s", exc)
        return
    except OSError as exc:
        msg = f"Failed to write export file:\n{exc}"
        show_colorful_error(parent, "File Error", msg)
        logger.exception("export_fd: file error: %s", exc)
        return

    show_colorful_info(
        parent,
        "Export Complete",
        f"Exported {exported_count} FD master record(s) to:\n{export_file}",
    )
...
def import_fd(
    ...
    """Import ``fd_master`` rows from ``all_fd_master.txt``.
    ...
    A row is **skipped** when an FD with the same ``fd_number`` already
    exists (unique constraint ``UNIQUE(fd_number)``).  The linked account is
    resolved by ``(bank_key, ac_number)``; the row is skipped (and logged)
    ...
    try:
        with open(_EXPORT_FILE, "r", encoding="utf-8") as fh, get_db_connection(
            ...
            for lineno, raw in enumerate(fh, start=1):
                ...
                try:
                    ...
                    logger.warning("import_fd: parse error line %d: %s", lineno, exc)
                    malformed += 1
                    continue

                if not isinstance(value, (list, tuple)) or len(value) < 9:
                    logger.warning("import_fd: short tuple line %d: %r", lineno, value)
                    malformed += 1
                    continue
                ...
                if row_type != "FD":
                    logger.warning(
                        "import_fd: unexpected row type %r on line %d",
                        row_type,
                        lineno,
                    )
                    malformed += 1
                ...
                if not fd_number or not open_dt or not maturity_dt:
                    logger.warning(
                        "import_fd: missing required field on line %d",
                        lineno,
                    )
                    malformed += 1
                    continue
                ...
                if account_id is None:
                    logger.warning(
                        "import_fd: skipping line %d — account not found"
                ...
                try:
                    # ── Duplicate check on fd_number (UNIQUE) ─────────────
                    cursor.execute(
                        "SELECT fd_master_id FROM fd_master"
                        " WHERE fd_number = ? LIMIT 1",
                        (fd_number,),
                    )
                    if cursor.fetchone():
                        skipped += 1
                    ...
                    cursor.execute(
                        ...
                        INSERT INTO fd_master
                            (account_id, fd_number, principal_amount,
                             interest_rate, open_dt, maturity_dt, is_active)
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                        """,
                        (
                    ...
                    logger.exception("import_fd: DB error on line %d: %s", lineno, exc)
        ...
        logger.exception("import_fd: file error: %s", exc)
    ...
    show_colorful_info(
        parent,
        "Import Complete",
        f"FD Master — inserted: {inserted}, skipped: {skipped}\n"
