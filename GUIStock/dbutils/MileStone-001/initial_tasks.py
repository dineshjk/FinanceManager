...
from GUIStock.config.globals import DB_PATH, logger
    ...
    try:
        with get_db_connection() as conn:
            ...
            cursor.execute(
                """
                INSERT INTO stocks (
                    stk_code, isin, company_name, sector, face_value, tick, is_active, is_etf
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    stk_code, isin, company_name, sector, face_value, tick, is_active, is_etf
            ...
            conn.commit()
        logger.info(f"Added company: {company_name} ({stk_code})")
        return True, f"Company '{company_name}' added successfully."
    except sqlite3.IntegrityError as e:
        logger.warning(f"Integrity error adding company: {company_name} ({stk_code}): {e}")
        return False, f"Integrity error: {e}"
    except Exception as e:
        logger.error(f"Error adding company: {company_name} ({stk_code}): {e}", exc_info=True)
