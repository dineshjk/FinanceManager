...
# from cmath import e  # Unused - not the mathematical constant
# from numbers import Rational  # Unused
# from random import gauss  # Unused
# from tabnanny import check  # Unused - check references are variable names
...
from GUIStock.config.globals import get_db_connection, ETCN, ETCB, BROK, GST, SEBI, STT
...
def disable_parent(parent: tk.Toplevel) -> None:
    """
    Disable a parent window to create modal behavior.

    Args:
        parent (tk.Toplevel): The parent window to disable.

    Side Effects:
        Sets the parent window's disabled attribute to True, preventing user interaction.
        If the operation fails, silently continues without raising an exception.
    """
    try:
...
def enable_parent(parent: tk.Toplevel) -> None:
    """
    Re-enable a previously disabled parent window and bring it to focus.

    Args:
        parent (tk.Toplevel): The parent window to re-enable.

    Side Effects:
        - Sets the parent window's disabled attribute to False, restoring user interaction
        - Attempts to deiconify (restore if minimized) and lift the window to the top
        - If any operation fails, silently continues without raising an exception
    """
    try:
...
def add_trade(parent: tk.Toplevel) -> None:
    """
    Opens a modal window for revised trade data entry, initializes form widgets, and manages parent window modality.
    Args:
    ...
    Side Effects:
        ...
        - Disables/enables the parent window for true modality.
    ...
    """
    #messagebox.showinfo("Info", "Revised Add Trade clicked", parent=parent)
    ...
    def on_ok() -> None:
        ...
        if current_trade_no < no_of_trades_var.get():
            ...
            messagebox.showinfo("Contract Complete",
            ...
            response = messagebox.askyesno("Add Another Trade?",
    ...
    rat_win.title("Revised Data Entry - Trade") # Title of the Window
    ...
    # Disable parent window for true modality
    ...
    # Heading
    header_frame = tk.Frame(rat_win, bg="navy")
    header_frame.pack(fill="x")
    tk.Label(header_frame, text="Data Entry", font=("Helvetica", 16, "bold"),
             bg="navy", fg="white", pady=5).pack(fill="x")
    # Second Line
    tk.Label(header_frame, text="Contract Details", font=("Helvetica", 14), bg="maroon", fg="white", pady=2).pack(fill="x")

    # rat_frame starting point
    # This is the area reserved for the form to contain widgets
    rat_frame = tk.Frame(rat_win, padx=10, pady=10)
    rat_frame.pack(fill="both", expand=True)

    # Create a two line frame to hold three fields
    cont_frame = tk.Frame(rat_frame)
    cont_frame.grid(row=1, column=0, rowspan=2, columnspan=5, sticky="ew", pady=7)

    # cont_frame starts
    # Label and Entry within cont_frame
    cont_label_frame = tk.Frame(cont_frame)
    cont_label_frame.pack(fill="x", pady=(0, 2))

    cont_entry_frame = tk.Frame(cont_frame)
    cont_entry_frame.pack(fill="x", pady=(0, 2))


    separator4 = tk.Frame(rat_frame, height=10, bd=10, relief="raised", bg="gold")
    separator4.grid(row=3, column=0, columnspan=4, sticky="ew", pady=8)

    # Create one line frame for company and isin
    company_isin_frame = tk.Frame(rat_frame)
    company_isin_frame.grid(row=4, column=0, columnspan=2, sticky="ew", pady=2)

    # Create a two line frame to hold three fields
    qoq_frame = tk.Frame(rat_frame)
    qoq_frame.grid(row=5, column=0, rowspan=2, columnspan=3, sticky="ew", pady=2)

    # Create a horizontal frame for the labels (first row)
    qoq_label_frame = tk.Frame(qoq_frame)
    qoq_label_frame.pack(fill="x", pady=(0, 2))

    # Create a horizontal frame for the entry widgets (second row)
    qoq_entry_frame = tk.Frame(qoq_frame)
    qoq_entry_frame.pack(fill="x", pady=(0, 2))

    # Create a two line frame to hold three fields
    rbnn_frame = tk.Frame(rat_frame)
    rbnn_frame.grid(row=7, column=0, rowspan=2, columnspan=4, sticky="ew", pady=2)

    # Create a horizontal frame for the labels
    rbnn_label_frame = tk.Frame(rbnn_frame)
    rbnn_label_frame.pack(fill="x", pady=(0, 2))

    # Create a horizontal frame for the entry widgets
    rbnn_entry_frame = tk.Frame(rbnn_frame)
    rbnn_entry_frame.pack(fill="x", pady=(0, 2))

    # Create a horizontal frame for the entry widgets
    wbt_frame = tk.Frame(rat_frame)
    wbt_frame.grid(row=9, column=0, rowspan=2, columnspan=4, sticky="ew", pady=2)

    wbt_label_frame = tk.Frame(wbt_frame)
    wbt_label_frame.pack(fill="x", pady=(0, 2))

    wbt_entry_frame = tk.Frame(wbt_frame)
    wbt_entry_frame.pack(fill="x", pady=(0, 2))

    # Create a horizontal frame for the entry widgets
    gss_frame = tk.Frame(rat_frame)
    gss_frame.grid(row=11, column=0, rowspan=2, columnspan=4, sticky="ew", pady=2)

    gss_label_frame = tk.Frame(gss_frame)
    gss_label_frame.pack(fill="x", pady=(0, 2))

    gss_entry_frame = tk.Frame(gss_frame)
...
# rbnn_frame starts here
    ...
    # --- Submit button field starts here ---
    submit_btn = tk.Button(rbnn_entry_frame, text="Submit", width=15, font=("Helvetica", 14), command=on_submit)
...
# rbnn_frame starts here
    ...
    btn_frame = tk.Frame(rat_win, pady=10)
    ...
    cancel_btn = tk.Button(btn_frame, text="Cancel", width=10, command=rat_win.destroy)
    ...
    # Set focus to Trade Menu button (Add Trade)
    try:
        for child in parent.winfo_children():
            if isinstance(child, tk.Button) and child.cget('text').lower().startswith('add trade'):
                child.focus_set()
                break
    except Exception:
        pass
...
# "navy" (already used, good contrast)
# "darkgreen"
# "maroon"
# "darkred"
