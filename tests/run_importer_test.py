...
def run():
    ...
    with mock.patch.object(_fd, "askopenfilename", _fake_askopenfilename):
        # The new UI handles auto-closing and choosing internally for tests.
        # We just need to call the function and then manage the root window.
        trade_entry_from_file(root)

        # Let Tk process events so the modal can appear and log
        try:
            # Auto-close the root window after a short delay to allow the test to finish
