...
import inspect
from .style_utils import ensure_ttk_style
from .globals import logger
from .modal_utils import disable_parent, enable_parent
from .window_manager import safe_close_modal
...
def create_menu_window(config):
    ...
    if not is_modal:
        ...
        window.grab_set()
        disable_parent(window)  # Disable parent window for true modal behavior
        # Ensure parent is re-enabled when the modal is closed
        window.protocol(
            "WM_DELETE_WINDOW",
            lambda: safe_close_modal(
                window, cleanup_func=lambda: enable_parent(window)
            ),
        )
                        ...
                        if next_btn.winfo_exists() and window.winfo_exists():
                            for b in buttons:
                                try:
                                    ...
                                    pass
                            # If a child modal window has grabbed events, do not
                            # highlight any button — the menu is not active.
                            try:
                                grab_owner = window.grab_current()
                                if grab_owner is not None and grab_owner != window:
                                    return
                            except tk.TclError:
                                return
                            next_btn.focus_set()
